"""Revision-guarded WordPress release for electric screwdrivers."""
...
SLUG = "best-electric-screwdrivers-for-diy-projects-in-2025"
POST_ID = 81
TITLE = "4V Electric Screwdrivers for Household Assembly"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare SKIL SD561201, BLACK+DECKER BDCS20C and WORX WX240L by speed control, charging, included bits and household-task limitations."
CONTENT = (ROOT / "handytested_phase4_electric_screwdrivers.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-electric-screwdrivers-backup"))
PRODUCTS = {"SKIL SD561201", "BLACK+DECKER BDCS20C", "WORX WX240L"}
RELATED = ("/best-home-diy-tool-sets-for-under-300-in-2025/", "/best-oscillating-multi-tools-under-75-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of electric-screwdriver article", flush=True)
