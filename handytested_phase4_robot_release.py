"""Revision-guarded WordPress release for the pet-hair robot-vacuum guide."""
...
POST_ID = 187
TITLE = "Robot Vacuums for Pet Hair: Two Documented Choices"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Roborock Q7 M5 and Shark Navigator RV2120AE for pet hair, mapping, brush maintenance, mopping and self-emptying."
CONTENT = (ROOT / "handytested_phase4_robot_vacuums.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-robot-backup"))
RELATED = (
...
def check_public(url: str) -> dict[str, str]:
    ...
    for marker in ("Roborock Q7 M5", "Shark Navigator RV2120AE"):
        if marker not in body:
            raise RuntimeError(f"Missing product: {marker}")
    if "Neato D4" in body or "Ecovacs Deebot N79S" in body:
        raise RuntimeError("Old lineup remains")
...
def main() -> None:
    ...
    products = {"Roborock Q7 M5", "Shark Navigator RV2120AE"}
    ...
    try:
        ...
        print("Release failed; attempting rollback of robot vacuum article", flush=True)
