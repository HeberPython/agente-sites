"""Revision-guarded WordPress release for the random orbital sander guide."""
...
SLUG = "best-random-orbital-sanders-for-diy-projects-2025"
POST_ID = 37
TITLE = "Random Orbital Sanders: Three Documented DIY Choices"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCW210B, Makita BO5041 and BLACK+DECKER BDERO100 random orbital sanders by power, speed control and dust options."
CONTENT = (ROOT / "handytested_phase4_sanders.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-sanders-backup"))
RELATED = (
...
def check_public(url: str) -> dict[str, str]:
    ...
    for marker in ("DEWALT DCW210B", "Makita BO5041", "BLACK+DECKER BDERO100"):
        if marker not in body:
            raise RuntimeError(f"Missing product: {marker}")
    if "We tested four popular models" in body or "captured over 98%" in body:
...
def main() -> None:
    ...
    products = {"DEWALT DCW210B", "Makita BO5041", "BLACK+DECKER BDERO100"}
    ...
    try:
        ...
        print("Release failed; attempting rollback of sander article", flush=True)
