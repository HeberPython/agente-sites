"""Revision-guarded WordPress release for the researched smart-home hub guide."""
...
POST_ID = 41
TITLE = "Smart Home Hubs: Alexa, Google Home and SmartThings"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Echo Hub, Nest Hub (2nd gen) and the original Aeotec Smart Home Hub by ecosystem, Matter, Thread, Zigbee and Z-Wave support."
CONTENT = (ROOT / "handytested_phase4_smart_hubs.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-smart-hubs-backup"))
RELATED = (
...
def check_public(url: str) -> dict[str, str]:
    ...
    for marker in ("Amazon Echo Hub", "Google Nest Hub (2nd gen)", "Aeotec Smart Home Hub"):
        if marker not in body:
            raise RuntimeError(f"Missing product: {marker}")
    if "2,100-square-foot home" in body or "response time averaged" in body:
...
def main() -> None:
    ...
    products = {"Amazon Echo Hub", "Google Nest Hub (2nd gen)", "Aeotec Smart Home Hub"}
    ...
    try:
        ...
        print("Release failed; attempting rollback of smart-home hub article", flush=True)
