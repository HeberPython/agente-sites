"""Revision-guarded WordPress release for electric lawn mowers."""
...
SLUG = "best-electric-lawn-mowers-under-300-for-2025"
POST_ID = 181
TITLE = "Electric Lawn Mowers: Cordless and Corded Choices for Small Yards"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Greenworks 25322 and BLACK+DECKER BEMW472BH by power, deck, kit contents and safe cord use for small lawns."
CONTENT = (ROOT / "handytested_phase4_mowers.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-mowers-backup"))
PRODUCTS = {"Greenworks 25322", "BLACK+DECKER BEMW472BH"}
RELATED = ("/best-home-diy-tool-sets-for-under-300-in-2025/", "/best-cordless-drills-under-100/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of mower article", flush=True)
