"""Revision-guarded WordPress release for home projectors."""
...
SLUG = "best-projectors-under-300-for-home-entertainment"
POST_ID = 156
TITLE = "Home Projectors: Native Resolution, Brightness and Setup"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Yaber Pro V9, Epson CO-W01 and ViewSonic PA503S by native resolution, manufacturer brightness, throw distance and streaming setup."
CONTENT = (ROOT / "handytested_phase4_projectors.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-projectors-backup"))
PRODUCTS = {"Yaber Pro V9", "Epson CO-W01", "ViewSonic PA503S"}
RELATED = ("/best-smart-tvs-under-300-for-2025-viewing-experience/", "/best-portable-outdoor-grills-under-300-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of projector article", flush=True)
