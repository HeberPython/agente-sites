"""Revision-guarded WordPress release for smart-home automation devices."""
...
POST_ID = 173
TITLE = "Smart Home Automation Devices: Four Different Starting Points"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Echo Hub, a Philips Hue Bridge Pro starter kit, August Wi-Fi Smart Lock and Kasa HS220 by compatibility, installation and real use case."
CONTENT = (ROOT / "handytested_phase4_automation.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-automation-backup"))
PRODUCTS = {"Amazon Echo Hub", "Philips Hue White and Color Ambiance", "August Wi-Fi Smart Lock", "Kasa HS220"}
RELATED = ("/best-smart-home-hubs-under-150-for-2025/", "/best-smart-home-devices-under-300-for-2025/")
...
def check_public(url: str) -> dict[str, str]:
    ...
    if source.count(TAG) < 4 or meta["canonical"] != url:
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of automation article", flush=True)
