"""Revision-guarded WordPress release for cordless brad nailers."""
...
POST_ID = 143
TITLE = "Cordless Brad Nailers for Trim: Compare Kits and Controls"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCN680B, RYOBI P321 and CRAFTSMAN CMCN618C1 18-gauge cordless brad nailers by battery package, controls and trim use."
CONTENT = (ROOT / "handytested_phase4_nailers.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-nailers-backup"))
PRODUCTS = {"DEWALT DCN680B", "RYOBI P321", "CRAFTSMAN CMCN618C1"}
RELATED = ("/best-home-diy-tool-sets-for-under-300-in-2025/", "/best-cordless-impact-wrenches-under-300-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of nailer article", flush=True)
