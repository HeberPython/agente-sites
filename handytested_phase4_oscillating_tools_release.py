"""Revision-guarded WordPress release for oscillating multi-tools."""
...
SLUG = "best-oscillating-multi-tools-under-75-for-2025"
POST_ID = 28
TITLE = "Oscillating Multi-Tools by Power Source and Kit"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCS356B, BLACK+DECKER BD200MTB and WEN 2312 by power source, speed control, accessory system and package contents."
CONTENT = (ROOT / "handytested_phase4_oscillating_tools.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-oscillating-tools-backup"))
PRODUCTS = {"DEWALT DCS356B", "BLACK+DECKER BD200MTB", "WEN 2312"}
RELATED = ("/best-home-diy-tool-sets-for-under-300-in-2025/", "/best-random-orbital-sanders-for-diy-projects-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of oscillating-tool article", flush=True)
