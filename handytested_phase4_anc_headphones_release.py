"""Revision-guarded WordPress release for ANC headphones."""
...
POST_ID = 72
TITLE = "Noise-Canceling Headphones by Travel Priority"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Sony WH-1000XM6, Bose QuietComfort Headphones and soundcore Space One Pro by codecs, wired use, folding design and battery claims."
CONTENT = (ROOT / "handytested_phase4_anc_headphones.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-anc-headphones-backup"))
PRODUCTS = {"Sony WH-1000XM6", "Bose QuietComfort Headphones", "soundcore Space One Pro A3062"}
RELATED = ("/best-wireless-earbuds-under-300-for-2025/", "/best-smart-tvs-under-300-for-2025-viewing-experience/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of ANC-headphones article", flush=True)
