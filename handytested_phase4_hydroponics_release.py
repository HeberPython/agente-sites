"""Revision-guarded WordPress release for the hydroponic-garden guide."""
...
POST_ID = 185
TITLE = "Indoor Hydroponic Gardens: Three Documented Countertop Choices"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare LetPot LPH-SE, iDOO I-HGH01 and LetPot LPH-Air by growing sites, tank, controls and ongoing supplies."
CONTENT = (ROOT / "handytested_phase4_hydroponics.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-hydroponics-backup"))
PRODUCTS = {"LetPot LPH-SE", "iDOO Translucent Garden", "LetPot LPH-Air"}
RELATED = ("/best-kitchen-gadgets-under-300-for-modern-chefs/", "/best-diy-outdoor-furniture-kits-under-300-for-2025/")
...
def check_public(url: str) -> dict[str, str]:
    ...
    if not all(product in body for product in PRODUCTS):
        raise RuntimeError("A documented product is missing")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of hydroponics article", flush=True)
