"""Revision-guarded WordPress release for rotary tools."""
...
SLUG = "best-rotary-tools-under-50-for-diy-projects-2025"; POST_ID = 47
TITLE = "Rotary Tools for General DIY and Detail Work"; SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Dremel 3000-1/25, WEN 2305 and Dremel Stylo+ 2050-15 by speed range, included accessories, flex-shaft support and intended workload."
CONTENT = (ROOT / "handytested_phase4_rotary_tools.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md"); SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run"); BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-rotary-tools-backup"))
PRODUCTS = {"Dremel 3000-1/25", "WEN 2305", "Dremel Stylo+ 2050-15"}
RELATED = ("/best-oscillating-multi-tools-under-75-for-2025/", "/best-electric-screwdrivers-for-diy-projects-in-2025/")
...
def check_public(url: str) -> dict[str, str]:
    ...
    if source.count(TAG) < 3 or meta["canonical"] != url or not all(p in source for p in RELATED): raise RuntimeError("Links or canonical missing")
    return meta
...
def main() -> None:
    ...
    baseline = next(p for p in json.loads(SNAPSHOT.read_text(encoding="utf-8"))["posts"] if p["slug"] == SLUG); current = api(f"/wp/v2/posts/{POST_ID}?context=edit")
    ...
    old_meta = public_meta(fetch_html(current["link"])); print(json.dumps({"mode": MODE, "post": SLUG, "new_title": TITLE, "validator": findings}, ensure_ascii=False))
    if old_meta["canonical"] != current["link"]: raise RuntimeError("Baseline canonical changed")
    if MODE == "dry-run": return
    BACKUP.mkdir(parents=True, exist_ok=True); (BACKUP / "original.json").write_text(json.dumps({"post": current, "public_meta": old_meta}, ensure_ascii=False, indent=2), encoding="utf-8")
