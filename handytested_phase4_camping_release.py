"""Revision-guarded WordPress release for car-camping gear."""
...
SLUG = "best-camping-gear-under-300-for-outdoor-adventures"
POST_ID = 141
TITLE = "Car-Camping Gear: Tent, Stove and Chair by Use"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Coleman Sundome 4-person tent, Coleman Triton two-burner stove and original Helinox Chair One by campsite role, variants and safety."
CONTENT = (ROOT / "handytested_phase4_camping.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-camping-backup"))
PRODUCTS = {"Coleman Sundome 4-person", "Coleman Triton", "Helinox original Chair One"}
RELATED = ("/best-portable-outdoor-grills-under-300-for-2025/", "/best-diy-outdoor-furniture-kits-under-300-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of camping article", flush=True)
