"""Revision-guarded WordPress release for smart-home devices."""
...
POST_ID = 168
TITLE = "Smart-Home Devices for Door, HVAC and Outlet Control"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Ring Battery Doorbell Plus 2nd Gen, ecobee Enhanced and Kasa KP125M by installation, subscription, platform and wiring requirements."
CONTENT = (ROOT / "handytested_phase4_smart_devices.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-smart-devices-backup"))
PRODUCTS = {"Ring Battery Doorbell Plus (2nd Gen)", "ecobee Smart Thermostat Enhanced", "Kasa KP125M"}
RELATED = ("/top-smart-home-automation-devices-under-300-for-2025/", "/best-smart-home-security-cameras-under-300-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of smart-devices article", flush=True)
