"""Revision-guarded WordPress release for pet-owner carpet cleaners."""
...
SLUG = "best-carpet-cleaners-for-pet-owners-in-2025"
POST_ID = 146
TITLE = "Carpet Cleaners for Pet Owners: Spot or Whole-Room?"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare BISSELL Little Green Pet Deluxe 3353, Hoover PowerDash Pet Advanced FH55000V and BISSELL HydroSteam Pet 3432 by cleaning area and model-specific features."
CONTENT = (ROOT / "handytested_phase4_carpet_cleaners.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-carpet-cleaners-backup"))
PRODUCTS = {"Little Green Pet Deluxe 3353", "PowerDash Pet Advanced FH55000V", "Revolution HydroSteam Pet 3432"}
RELATED = ("/best-robot-vacuums-for-pet-hair-under-300/", "/best-smart-home-devices-under-300-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of carpet-cleaner article", flush=True)
