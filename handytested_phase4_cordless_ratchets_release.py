"""Revision-guarded WordPress release for cordless ratchets."""
...
SLUG = "best-cordless-ratchets-for-diy-mechanics-in-2025"
POST_ID = 84
TITLE = "Cordless Ratchets by Battery System and Package"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCF513B, Milwaukee 2457-21 and CRAFTSMAN CMCF930B by torque, speed, battery platform, head access and package contents."
CONTENT = (ROOT / "handytested_phase4_cordless_ratchets.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-cordless-ratchets-backup"))
PRODUCTS = {"DEWALT DCF513B", "Milwaukee 2457-21", "CRAFTSMAN CMCF930B"}
RELATED = ("/best-cordless-impact-wrenches-under-300-for-2025/", "/best-home-diy-tool-sets-for-under-300-in-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of cordless-ratchet article", flush=True)
