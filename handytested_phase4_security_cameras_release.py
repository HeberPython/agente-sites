"""Revision-guarded WordPress release for security cameras."""
...
POST_ID = 136
TITLE = "Home Security Cameras by Location and Recording Needs"
...
CONTENT = (ROOT / "handytested_phase4_security_cameras.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-security-cameras-backup"))
PRODUCTS = {"Ring Indoor Cam 2K", "Blink Outdoor 4", "Arlo Essential Outdoor 2K"}
RELATED = ("/best-smart-home-devices-under-300-for-2025/", "/best-smart-home-hubs-under-150-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of security cameras article", flush=True)
