"""Revision-guarded WordPress release for kitchen gadgets."""
...
SLUG = "best-kitchen-gadgets-under-300-for-modern-chefs"
POST_ID = 171
TITLE = "Kitchen Gadgets for Cooking, Temperature Checks and Food Storage"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Instant Pot Duo 6QT, TP19H thermometer and FoodSaver VS3130 by actual kitchen task, model features and food-safety considerations."
CONTENT = (ROOT / "handytested_phase4_kitchen_gadgets.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-kitchen-gadgets-backup"))
PRODUCTS = {"Instant Pot Duo 6QT", "TempPro TP19H", "FoodSaver VS3130"}
RELATED = ("/best-high-performance-blenders-for-home-smoothies/", "/best-kitchen-appliances-under-300-for-home-chefs/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of kitchen gadgets article", flush=True)
