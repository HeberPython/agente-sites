...
| Homepage mobile LCP hero | CSS background discovered late, without image priority or responsive selection | Same visual delivered as a responsive decorative image with intrinsic dimensions and high fetch priority | `36246292734` / `10908105183` | Playwright mobile/desktop, PSI and 38/38 sitemap |
...
Follow-up `PerformanceObserver` instrumentation recorded shift sources before and after ad insertion at 320, 375 and 1280px. Large shifts were attributable to late Google Auto Ads at the top of `#page`, between Astra's header and `#content`: a `google-auto-placed` block of 300-380px appeared, and `#content` moved by the same amount. Examples: Cordless Drills 320px, content y=80→405, shift 0.329; Premium Earbuds 375px, y=80→460, shift 0.385; Tools 1280px, y=81→381, shift 0.216; correct Smart Home Hubs 320px, y=80→405, shift 0.329 on one load, versus no top ad and zero shift on another. Home `#top-picks` shifted after top-ad insertion at 375px (0.183) and 1280px (0.103). Some smaller shifts (about 0.002-0.017) affected an in-content paragraph, an archive card or ad element; their individual cause was not proven. This is **third-party top-placement CLS**, not evidence that the article image, font or footer caused the large shifts. Reserving a permanent 300-380px blank region when no ad appears would damage UX, so no speculative CSS was applied. INP was not measurable from these visits. An earlier PageSpeed request returned 429; a later focused run succeeded and is recorded below. No PSI result is treated as field CrUX data.

### 2026-09-26 homepage mobile LCP optimization

The supplied baseline was mobile score 68, FCP 3.9 s, LCP 6.4 s, TBT 50 ms and CLS 0; desktop was 94, FCP 0.9 s, LCP 1.3 s and CLS 0. A fresh pre-change Lighthouse sample identified `.ht-hero` as the LCP element and showed its CSS background request waiting about 890 ms before loading. The image was neither represented in initial HTML as an image request nor eligible for `fetchpriority`/responsive source selection.

Commit `866958a` added a guarded, manual-only release workflow and changed only WordPress page 18. It preserved the hero's source image, overlay, crop, text and dimensions while replacing the background-image delivery with a decorative responsive `<img>`: fixed width/height attributes, `srcset`, `sizes="100vw"`, `fetchpriority="high"`, `decoding="async"`, and no `loading="lazy"`. It did not change editorial copy, links, Amazon tracking, AdSense, Analytics, metadata, Rank Math, permalinks or sitemap configuration. Dry-run [36246259444](https://github.com/HeberPython/agente-sites/actions/runs/36246259444) passed. Apply [36246292734](https://github.com/HeberPython/agente-sites/actions/runs/36246292734) passed with original-page backup artifact `10908105183` and no rollback.

Playwright confirmed at 375px and the available desktop viewport that the image exactly covers the existing hero, preserves mobile/desktop object position, has high priority and no lazy loading, and introduces no horizontal overflow. Title, canonical, one H1, JSON-LD, disclosure, one AdSense loader, one Analytics loader and the 38/38 post sitemap remained intact. The post-change Lighthouse sample no longer reported LCP request discovery as a failed insight; resource-load delay was about 660 ms, approximately 230 ms lower than the paired pre-change sample. Mobile measured 66/FCP 3.2 s/LCP 6.0 s/TBT 270 ms/CLS 0; desktop measured 95/FCP 0.9 s/LCP 1.3 s/TBT 30 ms/CLS 0. The mobile FCP/LCP improved against the supplied baseline, while the score/TBT varied against the immediate pre-change run. This is one lab sample, not proof of a durable field improvement. Render-blocking Inter/Astra CSS, below-fold images and third-party scripts remain separately measurable; none was changed without a comparably low-risk, evidence-backed path.
...
- **P2 — performance follow-up:** repeat mobile PSI under consistent conditions and use Search Console/CrUX field data when available. Investigate Inter/Astra render blocking and below-fold image compression separately; preserve AdSense and Analytics unless direct evidence supports a sanctioned change.
