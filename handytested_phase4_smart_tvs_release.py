"""Revision-guarded WordPress release for smart TVs."""
...
SLUG = "best-smart-tvs-under-300-for-2025-viewing-experience"
POST_ID = 78
TITLE = "50-Inch Smart TVs by Platform and Inputs"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Amazon Fire TV 4-Series 4K50N402, Hisense 50A6N and Roku Select Series 50R4C5 by platform, HDR, gaming limits and inputs."
CONTENT = (ROOT / "handytested_phase4_smart_tvs.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-smart-tvs-backup"))
PRODUCTS = {"Amazon Fire TV 4-Series 4K50N402", "Hisense 50A6N", "Roku Select Series 50R4C5"}
RELATED = ("/best-projectors-under-300-for-home-entertainment/", "/best-smart-home-devices-under-300-for-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of smart-TV article", flush=True)
