"""Revision-guarded WordPress release for tool sets."""
...
SLUG = "best-home-diy-tool-sets-for-under-300-in-2025"
POST_ID = 90
TITLE = "Home DIY Tool Sets by Project Type"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare BLACK+DECKER BDPK70284C1AEV, DEWALT DCK240C2 and CRAFTSMAN CMMT45306 by household, fastening and mechanics tasks."
CONTENT = (ROOT / "handytested_phase4_tool_sets.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-tool-sets-backup"))
PRODUCTS = {"BLACK+DECKER BDPK70284C1AEV", "DEWALT DCK240C2", "CRAFTSMAN CMMT45306"}
RELATED = ("/best-cordless-drills-under-100/", "/best-cordless-nail-guns-under-300-for-diy-projects/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of tool sets article", flush=True)
