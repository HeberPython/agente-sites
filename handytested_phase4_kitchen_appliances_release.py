"""Revision-guarded WordPress release for kitchen appliances."""
...
POST_ID = 138
TITLE = "Kitchen Appliances by Task: Pressure, Blending and Coffee"
...
CONTENT = (ROOT / "handytested_phase4_kitchen_appliances.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-kitchen-appliances-backup"))
PRODUCTS = {"Instant Pot Duo Plus 6QT", "Ninja BN701", "Hamilton Beach FlexBrew Trio 49916G"}
RELATED = ("/best-kitchen-gadgets-under-300-for-modern-chefs/", "/best-high-performance-blenders-for-home-smoothies/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of kitchen appliances article", flush=True)
