"""Revision-guarded WordPress release for cordless circular saws."""
...
POST_ID = 59
TITLE = "6-1/2-Inch Cordless Circular Saws by Platform and Package"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCS565B, SKIL CR6413B-11 and Makita XSS02Z by blade speed, bevel range, battery platform and bare-tool versus kit package."
CONTENT = (ROOT / "handytested_phase4_circular_saws.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-circular-saws-backup"))
PRODUCTS = {"DEWALT DCS565B", "SKIL CR6413B-11", "Makita XSS02Z"}
RELATED = ("/best-home-diy-tool-sets-for-under-300-in-2025/", "/best-cordless-nail-guns-under-300-for-diy-projects/")
