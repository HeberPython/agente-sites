"""Revision-guarded WordPress release for cordless impact wrenches."""
...
SLUG = "best-cordless-impact-wrenches-under-300-for-2025"
POST_ID = 159
TITLE = "Cordless Impact Wrenches: Compact and Mid-Torque Options"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCF921B, Milwaukee 2962-20 and RYOBI P262 by anvil, manufacturer torque ratings, battery platform and tool-only package."
CONTENT = (ROOT / "handytested_phase4_impact_wrenches.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-impact-wrenches-backup"))
PRODUCTS = {"DEWALT DCF921B", "Milwaukee 2962-20", "RYOBI P262"}
RELATED = ("/top-5-digital-torque-wrenches-under-100-for-accurate-torque/", "/best-cordless-ratchets-for-diy-mechanics-in-2025/")
...
def check_public(url: str) -> dict[str, str]:
    ...
    if not all(product in body for product in PRODUCTS) or "garden cultivator" not in body:
        raise RuntimeError("Documented product/correction missing")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of impact-wrench article", flush=True)
