"""Revision-guarded WordPress release for cordless drills."""
...
POST_ID = 20
TITLE = "Cordless Drill Kits for Home DIY"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare DEWALT DCD771C2, BLACK+DECKER BDC120VA100 and CRAFTSMAN CMCD700C1 by chuck, speed, included batteries and kit contents."
CONTENT = (ROOT / "handytested_phase4_cordless_drills.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-cordless-drills-backup"))
PRODUCTS = {"DEWALT DCD771C2", "BLACK+DECKER BDC120VA100", "CRAFTSMAN CMCD700C1"}
RELATED = ("/best-cordless-circular-saws-under-150-for-diy-projects/", "/best-home-diy-tool-sets-for-under-300-in-2025/")
...
def main() -> None:
    ...
    print(json.dumps({"mode": MODE, "post": SLUG, "new_title": TITLE, "validator": findings}, ensure_ascii=False))
