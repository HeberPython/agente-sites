"""Revision-guarded WordPress release for laser levels."""
...
SLUG = "best-laser-levels-for-diy-home-projects-under-60"
POST_ID = 31
TITLE = "Laser Levels for Cross-Line and Wall Layout"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Bosch GLL50-20, DEWALT DW088CG and BLACK+DECKER BDL220S by self-leveling, beam color, range, mounting and detector limits."
CONTENT = (ROOT / "handytested_phase4_laser_levels.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-laser-levels-backup"))
PRODUCTS = {"Bosch GLL50-20", "DEWALT DW088CG", "BLACK+DECKER BDL220S"}
RELATED = ("/best-home-diy-tool-sets-for-under-300-in-2025/", "/best-stud-finders-under-50-for-home-projects-2025/")
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of laser-level article", flush=True)
