"""Revision-guarded WordPress release for portable outdoor grills."""
...
POST_ID = 183
TITLE = "Portable Outdoor Grills: Propane vs. Charcoal for Camping"
SEO_TITLE = TITLE + " | HandyTested"
DESCRIPTION = "Compare Coleman RoadTrip 225 Tabletop and Weber Go-Anywhere Charcoal by fuel, cooking area, carry weight and campsite safety."
CONTENT = (ROOT / "handytested_phase4_grills.html").read_text(encoding="utf-8")
SOURCE = ROOT / "handytested_sources" / (SLUG + ".md")
SNAPSHOT = ROOT / "handytested_sources" / "public_snapshot.json"
MODE = os.getenv("MODE", "dry-run")
BACKUP = Path(os.getenv("BACKUP_DIR", "handytested-phase4-grills-backup"))
PRODUCTS = {"Coleman RoadTrip 225 Tabletop", "Weber Go-Anywhere Charcoal"}
RELATED = ("/best-camping-gear-under-300-for-outdoor-adventures/", "/best-diy-outdoor-furniture-kits-under-300-for-2025/")
...
def check_public(url: str) -> dict[str, str]:
    ...
    if not all(product in body for product in PRODUCTS) or "CGG-180T" not in body:
        raise RuntimeError("Product decision missing")
    if source.count(TAG) < 2 or meta["canonical"] != url:
...
def main() -> None:
    ...
    try:
        ...
        print("Release failed; attempting rollback of portable grills article", flush=True)
